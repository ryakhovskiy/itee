...
public class Stats2DbSaver implements AutoCloseable {
    ...
    private final long time = Timer.currentTimeMillis();
    ...
    private void parsePwLine16(PreparedStatement statement, String[] data) throws ParseException, SQLException {
        ...
        final long rdtsc = Long.parseLong(data[1]);
        final double elapsedTime = Double.parseDouble(data[2]);
        final int iafreq = Integer.parseInt(data[3]);
        double ppw = Double.parseDouble(data[4]);
        double cpej = Double.parseDouble(data[5]);
        double cpew = Double.parseDouble(data[6]);
        double iapw = Double.parseDouble(data[7]);
        double ciaej = Double.parseDouble(data[8]);
        double ciaew = Double.parseDouble(data[9]);
        double gtpw = Double.parseDouble(data[10]);
        double cgtej = Double.parseDouble(data[11]);
        double cgtew = Double.parseDouble(data[12]);
        double drampw = Double.parseDouble(data[13]);
        double cdramej = Double.parseDouble(data[14]);
        double cdramew = Double.parseDouble(data[15]);
    ...
    public void saveServiceMemoryInfo(List<String> lines) throws ParseException, SQLException {
        final SimpleDateFormat kpiTimestampFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS");
        if (lines.size() <= 1)
            return;
        PreparedStatement statement = null;
        try {
            ...
            for (String line : lines) {
                ...
                for (int i = 2; i < data.length; i++) {
                    final long value = Long.parseLong(data[i]);
    ...
    public void saveExpensiveStatementsInfo(List<String> lines) throws SQLException {
        ...
        try {
            ...
            for (String line : lines) {
                ...
                statement.setInt(7, Integer.parseInt(data[5]));
                statement.addBatch();
            }
            statement.executeBatch();
        } finally {
        ...
        try {
            ...
            for (int i = 1; i < lines.length; i++) {
                ...
                for (int j = 1; j < data.length; j++) {
                    ...
                    if (data[j].indexOf(',') > 0)
                        data[j] = data[j].substring(0, data[j].indexOf(','));
                    long kpiValue = Long.parseLong(data[j]);
                    statement.setInt(1, sid);
    ...
    private void checkSchema(Connection connection) throws SQLException {
        ...
        try (Statement statement = connection.createStatement();
            PreparedStatement pstmnt = connection.prepareStatement("select * from schemas where schema_name = ?")) {
    ...
    private int getNextSidAndSave() throws SQLException {
        ResultSet set = null;
        try (Statement statement = connection.createStatement();
             PreparedStatement pstmnt = connection.prepareStatement("insert into " + SCHEMA + ".stats_info values (?,?,?,?)")) {
